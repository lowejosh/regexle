...
describe("gameStore", () => {
  ...
  describe("loadRandomPuzzle", () => {
    ...
    it("should handle puzzle loading error gracefully", async () => {
      ...
      const consoleSpy = vi
        .spyOn(console, "error")
        .mockImplementation(() => {});
      vi.mocked(puzzleLoader.getRandomPuzzle).mockRejectedValue(
  ...
  describe("testPattern", () => {
    ...
    it("should test pattern and update results", async () => {
      ...
      expect(RegexGameEngine.testPattern).toHaveBeenCalledWith(
        "test pattern",
        mockPuzzle.testCases
      );
  ...
  describe("practice puzzle state persistence", () => {
    ...
    it("should auto-save practice state when updating pattern", () => {
      ...
      expect(state.practicePuzzleStates[mockPuzzle.id].userPattern).toBe(
        "new.*pattern"
      );
    });
