...
function validateSolution(puzzle: Puzzle) {
  ...
  try {
    ...
    throw new Error(`Invalid regex pattern: ${puzzle.solution}. Error: ${error}`);
  }
...
describe('Puzzle Validation', () => {
      ...
      describe(`Puzzle: ${manifestEntry.id} - ${manifestEntry.title}`, () => {
        ...
        it('should have correct test case counts', () => {
          ...
          if (!validation.isMatchingCountValid) {
            console.error(`${puzzle.id}: Expected ${TEST_CASE_LIMITS.MIN_MATCHING}-${TEST_CASE_LIMITS.MAX_MATCHING} matching cases, got ${validation.matchingCount}`);
          }
          if (!validation.isNonMatchingCountValid) {
            console.error(`${puzzle.id}: Expected ${TEST_CASE_LIMITS.MIN_NON_MATCHING}-${TEST_CASE_LIMITS.MAX_NON_MATCHING} non-matching cases, got ${validation.nonMatchingCount}`);
          }
        });
        ...
        it('should have solution that passes all test cases', () => {
          ...
          if (!validation.isValid) {
            ...
            validation.failures.forEach(failure => {
              ...
              console.error(`  Expected: ${failure.expectedMatch ? 'MATCH' : 'NO MATCH'}`);
              console.error(`  Actual: ${failure.actualMatch ? 'MATCH' : 'NO MATCH'}`);
            });
          }
  ...
  describe('Cross-Puzzle Statistics', () => {
    it('should provide test suite summary', () => {
      ...
      stats.forEach(stat => {
        console.log(`${stat.id} (${stat.difficulty}): ${stat.matchingTests}/${stat.nonMatchingTests} (match/no-match)`);
      });
